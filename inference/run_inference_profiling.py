...
# This setting is only used for cores with (1) no input bit slicing, (2) no weight bit slicing, (3) no partitioning, (4) digital bias
# Make sure profile_ADC_inputs is true
profile_ADC_biased = True
...
# If False:
#   The same images will be used for calibration as used for a normal simulation, 
#   e.g. same images will be loaded in order (not recommended)
calibration = True
...
if profile_DAC_inputs and config.dac_bits > 0:
    raise ValueError("Activation bits should be zero when profiling ADC inputs")
...
if profile_ADC_inputs:
    ibit_msg = ("_ibits" if config.input_bitslicing and config.ADC_per_ibit else "")
    bias_msg = ("_bias" if profile_ADC_biased else "")
    profiling_folder = "../adc/adc_dac_inputs/ADCinputs_"+str(config.task)+"_"+str(config.model_name)+"_"+str(config.NrowsMax)+"rows_"+str(config.Nslices)+"slices_"+config.style+ibit_msg+bias_msg+"/"

elif profile_DAC_inputs:
    profiling_folder = "../adc/adc_dac_inputs/DACinputs_"+str(config.task)+"_"+str(config.model_name)+"/"
...
accuracy = inference(ntest=config.ntest,
    ...
    profiling_settings=[profile_DAC_inputs,profile_ADC_inputs,profile_ADC_biased])