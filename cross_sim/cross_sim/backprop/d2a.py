...
# digital to analog conversion models, implemented numerically
...
class D2A:
  """
  activation models
  style = BASIC = clip/discretize, then add noise
  """
  ...
  def __init__(self,**kwargs):
    ...
    if self.noise and (self.sigma < 0.0 or self.sigma > 1.0):
      error("Invalid d2a:sigma arg")
    if self.discrete and self.cliplo >= self.cliphi:
      error("Invalid d2a:clip args")
  ...
  def apply(self,x):
    ...
    if style == BASIC:
      ...
      # add noise last, proportional to cliprange
