...
profiling_folder = "./profiled_adc_inputs/imagenet_Resnet50-v1.5_"+\
			str(NrowsMax)+"rows_"+str(Nslices)+"slices_"+xbar_style+ibit_msg+"/"

# npy output file path
output_name = "./adc_limits/examples/adc_limits_ResNet50v15_"+\
			str(NrowsMax)+"rows_"+str(Nslices)+"slices_"+xbar_style+ibit_msg+".npy"
...
ADC_limits = np.zeros((len(layerNums),Nslices,2))
...
for k in range(len(layerNums)):
	...
	for i_slice in range(Nslices):
		...
		if k == 0 or xbar_style == "BALANCED":
			ADC_limits[k,i_slice,0] = -ymax / 2**clip_power_k
			ADC_limits[k,i_slice,1] = ymax / 2**clip_power_k
		else:
			ADC_limits[k,i_slice,0] = 0
			ADC_limits[k,i_slice,1] = ymax / 2**clip_power_k
