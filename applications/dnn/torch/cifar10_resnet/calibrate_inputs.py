...
from simulator.algorithms.dnn.torch.convert import (
    from_torch,
    convertible_modules,
)
...
resnet_model.load_state_dict(
    torch.load(
        "./models/resnet{:d}_cifar10.pth".format(depth),
...
for inputs, labels in cifar10_dataloader:
    ...
    if print_progress:
        print(
            "Image {:d}/{:d}, accuracy so far = {:.2f}%".format(
                k, N, 100 * np.sum(y[:k] == y_pred[:k]) / k
            ),
            end="\r",
        )
...
# Manually calibrate first layer's limits based on value range of CIFAR-10
# images
calibrated_ranges[0, :] = np.array([-2.64, 2.64])

np.save(
    "./calibrated_config/input_limits_ResNet{:d}.npy".format(depth), calibrated_ranges
)
