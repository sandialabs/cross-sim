    ...
    if adc_bits > 0 and adc_range_option == "CALIBRATED":
        if Nslices == 1:
            if (
                ...
                adc_ranges = np.load(
                    limits_dir + "adc_limits_ResNet{:d}_balanced.npy".format(depth)
                )
                ...
                adc_ranges = np.load(
                    limits_dir
                    + "adc_limits_ResNet{:d}_balanced_ibits.npy".format(depth)
                )

            elif style == "OFFSET" and NrowsMax >= 576 and adc_per_ibit:
                adc_ranges = np.load(
                    limits_dir + "adc_limits_ResNet{:d}_offset.npy".format(depth)
                )
        else:
            raise ValueError(
                "No calibrated ADC range found for the combination of crossbar style, "
                "adc_per_ibit and # slices"
            )
