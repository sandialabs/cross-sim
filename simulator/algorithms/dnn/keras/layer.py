...
class AnalogLayer(Layer, ABC):
    ...
    Implementing  classes should inherit from the original keras layer and
    AnalogLayer as follows `AnalogDense(AnalogLayer, keras.layers.Dense)` with
    the keras base layer last to ensure proper module resolution order.
    Implementing classes should exactly match the output of the original keras
    layer when all error models are disabled for the forward function. Keras
    AnalogLayers do not currently support backward operations.

    Attributes:
        core:
            An AnalogCore or similar object for CrossSim simulation. self.core
            must provide a set_matrix and get_matrix function. In most cases
            these should be thin wrappers around the implementations in
            AnalogCore
        params:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Generic AnalogLayer handling, passes kwargs to the base keras layer for
        further initialization. kwargs should contain all arguments for the
        keras layer.
        ...
        if self.useGPU:
            # There is a potential race condition because Keras doesn't use the
            # default stream for weights so the dlpack zero-copy transfer can
            # create a data race. Grab the stream here so we can use it
            # synchronize with it later.
            import cupy as cp
    ...
    def set_weights(self, weights: list[npt.NDArray]) -> None:
        ...
        Wraps Layer.set_weights function setting the matrix based on the new
        weights.
    ...
    def build(self, input_shape) -> None:
        ...
        Base class build will create weight variables based on the input shape
        and initialize those variables. Subclass implementations should create
        self.core in their implementations of build since the shape of the
        weight matrix isn't known until build is called.
    ...
    def get_core_weights(self) -> list[npt.NDArray]:
        ...
        Implementations should use self.get_matrix and reshape the result into
    ...
    def from_keras(
        ...
        Arguments:
            ...
            bias_rows:
                Integer indictating the number of analog rows to use for the
