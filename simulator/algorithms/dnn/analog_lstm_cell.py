...
"""CrossSim implementation of LSTM (Long Short-Term Memory) cell layers."""
...
class AnalogLSTMCell(AnalogLayer):
    """CrossSim implementation for Long Short-Term Memory Cell layers."""
    ...
    def __init__(
        ...
        """Initializes an LSTMCell layer wrapper around AnalogCore.

        Args:
            params:
                CrossSimParameters object or list of CrossSimParameters (for
                layers requiring multiple arrays) for the AnalogLSTMCell layer.
                If a list, the length must match the number of arrays used
                within AnalogCore.
            input_size:
                Number of input features, see torch.nn.LSTMCell input_size for
                detailed documentation.
            hidden_size:
                Number of hidden features, see torch.nn.LSTMCell hidden_size for
                detailed documentation.
            bias_ih_rows:
        ...
        # For LSTMCell, we need to handle 4 gates (input, forget, cell, output)
        # Each gate needs both input-to-hidden and hidden-to-hidden weights
        # The matrix will contain both weight_ih and weight_hh concatenated
        total_input_size = input_size + hidden_size

        total_cols = total_input_size + self.bias_ih_rows + self.bias_hh_rows

        self.weight_ih_mask = (slice(0, 4 * hidden_size, 1), slice(0, input_size, 1))
    ...
    def form_matrix(
        ...
        if w_ih.shape != (4 * self.hidden_size, self.input_size):
            raise ValueError(
                f"Expected weight_ih shape ({4 * self.hidden_size}, "
                f"{self.input_size}), got {w_ih.shape}"
            )

        if w_hh.shape != (4 * self.hidden_size, self.hidden_size):
            raise ValueError(
                f"Expected weight_hh shape ({4 * self.hidden_size}, "
        ...
        if bias_ih is not None and self.bias_ih_rows:
            ...
            matrix[:, bias_start : bias_start + self.bias_ih_rows] = np.repeat(
                ...
                self.bias_ih_rows,
                axis=0,
            ).reshape(4 * self.hidden_size, self.bias_ih_rows)
        ...
        if bias_hh is not None and self.bias_hh_rows:
            ...
            matrix[:, bias_start : bias_start + self.bias_hh_rows] = np.repeat(
                ...
                self.bias_hh_rows,
                axis=0,
            ).reshape(4 * self.hidden_size, self.bias_hh_rows)
    ...
    def apply(
        self, M_input: npt.ArrayLike, hx: npt.ArrayLike, cx: npt.ArrayLike
    ) -> tuple[npt.NDArray, npt.NDArray]:
        """Apply a single LSTM step using analog computation.

        Args:
            M_input:
                1D or 2D numpy NDArray of shape (input_size) or (batch_size,
                input_size).
            hx:
                1D or 2D numpy NDArray of shape (hidden_size) or (batch_size,
                hidden_size) for previous hidden state.
            cx:
                1D or 2D numpy NDArray of shape (hidden_size) or (batch_size,
                hidden_size) for previous cell state.

        Returns:
            Tuple of (new_hidden_state, new_cell_state), both as numpy NDArrays.
        """
        ...
        if cx.shape != (batch_size, self.hidden_size):
            raise ValueError(
                f"Expected cell shape ({batch_size}, {self.hidden_size}), "
                f"got {cx.shape}"
            )

        # Concatenate input and hidden state: [M_input, hx]
        ...
        # Core matrix shape: (4 * hidden_size,
        #   input_size + hidden_size + 2 * bias_rows)
        # Combined_input shape: (batch_size,
        #   input_size + hidden_size + 2 * bias_rows)
        # Core @ combined_input.T shape:(4 * hidden_size, batch_size)
        # We then transpose to get (batch_size, 4 * hidden_size)
