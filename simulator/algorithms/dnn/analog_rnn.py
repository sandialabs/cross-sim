...
"""CrossSim implementation of RNN (Recurrent Neural Network) layers."""
...
from .analog_rnn_cell import AnalogRNNCell
...
class AnalogRNN(AnalogLayer):
    """CrossSim implementation for RNN (Recurrent Neural Network) layers."""
    ...
    def __init__(
        ...
        """Initializes an RNN layer containing multiple AnalogRNNCell layers.

        Args:
            params:
                CrossSimParameters object or list of CrossSimParameters (for
                layers requiring multiple arrays) for the AnalogRNN layer. If a
        ...
        # Create AnalogRNNCell instances for each layer
        self.cells = []
        for layer in range(num_layers):
            ...
            cell = AnalogRNNCell(
                ...
                hidden_size,
                nonlinearity,
                bias_ih_rows if isinstance(bias_ih_rows, int) else bias_ih_rows[layer],
                bias_hh_rows if isinstance(bias_hh_rows, int) else bias_hh_rows[layer],
            )
            self.cells.append(cell)

    def form_matrix(
        self,
        weight_ih: list[npt.ArrayLike],
        weight_hh: list[npt.ArrayLike],
        bias_ih: list[npt.ArrayLike | None] = None,
        bias_hh: list[npt.ArrayLike | None] = None,
    ) -> list[npt.NDArray]:
        ...
        Returns:
            List of 2D numpy NDArray of the matrix for the specified RNN.
        """
    ...
    def apply(
        ...
        All digital processing (loops, activations, RNN logic) is handled
        ...
        Returns:
            Raw pre-activation outputs before activation function
        """
    ...
    def get_matrix(self) -> list[npt.NDArray]:
        ...
        """
        return [self.cells[layer].get_matrix() for layer in range(self.num_layers)]
    ...
    def set_matrix(self, matrix: list[npt.ArrayLike], verbose: bool = False) -> None:
        ...
        """
        for layer in range(self.num_layers):
            self.cells[layer].set_matrix(matrix[layer], verbose)
    ...
    def __getitem__(self, layer_idx: int) -> AnalogRNNCell:
        """Get a specific RNN cell by layer index.

        Args:
            layer_idx: Index of the layer to retrieve.

        Returns:
            AnalogRNNCell for the specified layer.
        """
        if layer_idx >= self.num_layers:
            raise ValueError(
                f"Layer index {layer_idx} is out of range for {self.num_layers} layers"
    ...
    def __getattr__(self, name):
        ...
        if var not in AnalogRNN._variable_names:
