...
"""CrossSim version of Torch.nn.GRU.

AnalogGRU provides a CrossSim-based forward using analog MVM.
"""

from __future__ import annotations

from .layer import AnalogLayer
from .gru_cell import AnalogGRUCellGrad

from simulator import CrossSimParameters
from simulator.algorithms.dnn.analog_gru import AnalogGRU as GRUCore
from torch import Tensor, from_dlpack, zeros
from torch.nn import GRU, Parameter
import torch


class AnalogGRU(GRU, AnalogLayer):
    """CrossSim implementation of torch.nn.GRU.

    See AnalogLayer for description of CrossSim-specific documentation.
    See torch.nn.GRU for layer functionality documentation.
    """

    def __init__(
        self,
        params: CrossSimParameters | list[CrossSimParameters],
        # Base GRU layer arguments
        input_size: int,
        hidden_size: int,
        num_layers: int = 1,
        bias: bool = True,
        ...
        device=None,
        dtype=None,
        # Additional arguments for AnalogGRU specifically
        bias_ih_rows: int | list[int] = 0,
        bias_hh_rows: int | list[int] = 0,
    ) -> None:
        """Initializes AnalogGRU and underlying torch.nn.GRU layer.
        ...
        Args:
            params:
                ...
                within AnalogCore.
            input_size: See torch.nn.GRU input_size argument.
            hidden_size: See torch.nn.GRU hidden_size argument.
            num_layers: See torch.nn.GRU num_layers argument.
            bias: See torch.nn.GRU bias argument.
            batch_first: See torch.nn.GRU batch_first argument.
            dropout: See torch.nn.GRU dropout argument.
            bidirectional: See torch.nn.GRU bidirectional argument.
            device: See torch.nn.GRU device argument.
            dtype: See torch.nn.GRU dtype argument.
            bias_ih_rows:
        ...
        """
        if bidirectional:
            raise NotImplementedError("Bidirectional AnalogGRU is not supported yet.")
    ...
    def forward(  # noqa: C901
        ...
        """GRU forward operation using per-cell analog computation.

        Processes the input sequence by iterating over timesteps and layers,
        delegating each (timestep, layer) step to AnalogGRUCellGrad. This
        ...
        activations, GRU recurrence) are performed digitally in PyTorch.
        ...
        if M_input.dim() not in (2, 3):
            raise ValueError(
                f"GRU: Expected input to be 2D or 3D, got {M_input.dim()}D instead"
            )
        ...
        for t in range(seq_len):
            ...
            for layer_idx in range(self.num_layers):
                ...
                bias_ih = getattr(self, f"bias_ih_l{layer_idx}") if self.bias else None
                bias_hh = getattr(self, f"bias_hh_l{layer_idx}") if self.bias else None

                # Use the cell-level autograd function
                h_current[layer_idx] = AnalogGRUCellGrad.apply(
                    layer_input,
                    h_current[layer_idx],
                    weight_ih,
                    weight_hh,
    ...
    def from_torch(
        ...
        """Build AnalogGRU from a GRU layer.

        Args:
            layer: torch.nn.GRU layer to copy.
            params:
        ...
        Returns:
            AnalogGRU layer with the same weights and properties as input
            GRU layer.
        """
    ...
    def to_torch(  # noqa:C901
        ...
        """Creates a torch GRU layer from an AnalogGRU.

        Args:
            layer: AnalogGRU layer to copy.
            physical_weights:
        ...
        Returns:
            torch.nn.GRU with the same properties and weights as the AnalogGRU
            layer.
        """
        if not device:
