...
"""Utility functions for collecting profiled data from CrossSim Torch layers.
...
def get_profiled_xbar_inputs(
    ...
    for k, layer in enumerate(analog_modules(model)):
        if layer.core.params.simulation.analytics.profile_xbar_inputs:
            if hasattr(layer.core, "cells"):  # RNN, LSTM, or GRU layer
                for cell in layer.core.cells:
                    if all_xbar_inputs[k] is None:
                        all_xbar_inputs[k] = cell.xbar_inputs.reshape(-1)
                    else:
                        all_xbar_inputs[k] = xp.concatenate(
                            (all_xbar_inputs[k], cell.xbar_inputs.reshape(-1)),
                            axis=0,
                        )
            else:
    ...
    if save_dir is not None:
        ...
        with save_path.open("wb") as sp:
            pickle.dump(all_xbar_inputs, sp)
...
def get_profiled_adc_inputs(  # noqa:C901
                ...
                for r in range(Ncores_r):
                    for c in range(Ncores_c):
                        all_adc_inputs_k = xp.concatenate(
                            (all_adc_inputs_k, cores[r][c].adc_inputs),
                            axis=1,
                        )
                    ...
                    for r in range(Ncores_r):
                        for c in range(Ncores_c):
                            bitslice_inputs_j = xp.concatenate(
                                (bitslice_inputs_j, cores[r][c].adc_inputs[j, :, :]),
                                axis=1,
                            )
                    if j == 0:
    ...
    if save_dir is not None:
        ...
        with save_path.open("wb") as sp:
            pickle.dump(all_adc_inputs, sp)

    return all_adc_inputs


def get_conductance_matrices(model: Module) -> npt.ArrayLike:  # noqa:C901
