...
"""CrossSim version of Torch.nn.RNN.

AnalogRNN provides a CrossSim-based forward using analog MVM.
"""

from __future__ import annotations

from .layer import AnalogLayer
from .rnn_cell import AnalogRNNCellGrad

from simulator import CrossSimParameters
from simulator.algorithms.dnn.analog_rnn import AnalogRNN as RNNCore
from torch import Tensor, from_dlpack, zeros
from torch.nn import RNN, Parameter
import torch


class AnalogRNN(RNN, AnalogLayer):
    """CrossSim implementation of torch.nn.RNN.

    See AnalogLayer for description of CrossSim-specific documentation.
    See torch.nn.RNN for layer functionality documentation.
    """

    def __init__(
        self,
        params: CrossSimParameters | list[CrossSimParameters],
        # Base RNN layer arguments
        input_size: int,
        hidden_size: int,
        num_layers: int = 1,
        ...
        device=None,
        dtype=None,
        # Additional arguments for AnalogRNN specifically
        bias_ih_rows: int | list[int] = 0,
        bias_hh_rows: int | list[int] = 0,
    ) -> None:
        """Initializes AnalogRNN and underlying torch.nn.RNN layer.
        ...
        Args:
            params:
                ...
                within AnalogCore.
            input_size: See torch.nn.RNN input_size argument.
            hidden_size: See torch.nn.RNN hidden_size argument.
            num_layers: See torch.nn.RNN num_layers argument.
            nonlinearity: See torch.nn.RNN nonlinearity argument.
            bias: See torch.nn.RNN bias argument.
            batch_first: See torch.nn.RNN batch_first argument.
            dropout: See torch.nn.RNN dropout argument.
            bidirectional: See torch.nn.RNN bidirectional argument.
            device: See torch.nn.RNN device argument.
            dtype: See torch.nn.RNN dtype argument.
            bias_ih_rows:
        ...
        """
        if bidirectional:
            raise NotImplementedError("Bidirectional AnalogRNN is not supported yet.")
    ...
    def forward(  # noqa:C901
        self, M_input: Tensor, hx: Tensor | None = None
    ) -> tuple[Tensor, Tensor]:
        """RNN forward operation using per-cell analog computation.

        Processes the input sequence by iterating over timesteps and layers,
        delegating each (timestep, layer) step to AnalogRNNCellGrad. This
        ...
        All other operations (sequencing, layer stacking, dropout, activation)
        are performed digitally in PyTorch.
        ...
        if M_input.dim() not in (2, 3):
            raise ValueError(
                f"RNN: Expected input to be 2D or 3D, got {M_input.dim()}D instead"
            )
        ...
        max_batch_size = M_input.size(batch_dim)
        ...
        if hx is None:
            ...
            if is_batched:
                if hx.dim() != 3:
                    raise RuntimeError(
                        f"For batched 3-D input, h0 should also be 3-D "
                        f"but got {hx.dim()}-D tensor"
                    )
            else:
                if hx.dim() != 2:
                    raise RuntimeError(
                        f"For unbatched 2-D input, h0 should also be 2-D "
                        f"but got {hx.dim()}-D tensor"
                    )
                hx = hx.unsqueeze(1)
        ...
        if hx.shape != (self.num_layers, batch_size, self.hidden_size):
            raise ValueError(
                f"Expected h0 shape ({self.num_layers}, {batch_size}, "
                f"{self.hidden_size}), got {hx.shape}"
            )

        # Current hidden state for all layers
        h_current = [hx[layer_idx] for layer_idx in range(self.num_layers)]

        outputs = []

        # Process each timestep — each cell call builds autograd graph
        for t in range(seq_len):
            ...
            for layer_idx in range(self.num_layers):
                ...
                h_current[layer_idx] = AnalogRNNCellGrad.apply(
                    layer_input,
                    h_current[layer_idx],
                    weight_ih,
                    weight_hh,
    ...
    def from_torch(
        ...
        """Build AnalogRNN from an RNN layer.

        Args:
            layer: torch.nn.RNN layer to copy.
            params:
        ...
        Returns:
            AnalogRNN layer with the same weights and properties as input
            RNN layer.
        """
    ...
    def to_torch(  # noqa:C901
        ...
        """Creates a torch RNN layer from an AnalogRNN.

        Args:
            layer: AnalogRNN layer to copy.
            physical_weights:
        ...
        Returns:
            torch.nn.RNN with the same properties and weights as the AnalogRNN
            layer.
        """
        if not device:
