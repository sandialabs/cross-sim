...
class BaseParameters:
    ...
    def __new__(cls, *args, **kwargs) -> BaseParameters:
        ...
        # (i.e. It was automatically created because 'None' was provided during
        # init)
    ...
    def parent(self) -> BaseParameters:
        ...
        Returns:
            BaseParameters: Returns the Parameters object that holds this
    ...
    def parents(self) -> list[BaseParameters]:
        """Returns a list of parents of the parameters.
    ...
    def validate(self) -> None:
        ...
        Will warn if values do not match type hints.
        ...
        for key, value in self.__dataclass_fields__.items():
            ...
            # TODO: This could be more robust, but I'm not worrying about that
            # as I'm not sure if we want this feature
            #       - This line behaves as follows
            #       - list[int], etc. -> Checks if the object is an int.
            #           Does not check if the elements are ints
            #       - Union[int, float], etc. -> This is not handled and it will
            #           cause an error
            # If we want to keep this type checking we might want to just wrap
            #   this in a try/except
            # This type validation could also probably be moved inside
            #   __setattr__ which would make it automatic
            value_type = _resolve_type(value.type, self.__module__)
            if not isinstance(getattr(self, key), value_type):
                message = (
                    f"Key {key} must be of type '{value.type}', "
                    f"got {type(getattr(self, key))}"
                )
                log.debug(message)
    ...
    def update(self, update: dict[str, Any]) -> None:
        ...
        Args:
            update (dict[str, Any]): Values to update in the parameters. Keys in
                the dictionary are the value of the field to update. Recursive
                updates can be performed using 'dot' notation (ex: foo.bar)
        """
    ...
    def as_dict(self) -> dict[str, Any]:
        """Return dataclass as a dict using the built in dataclasses.asdict.
    ...
    def to_json(self, filepath: Union[str, bytes, os.PathLike], **kwargs) -> None:
        ...
        """
        with Path.open(filepath, "w", **kwargs) as outfile:
    ...
    def from_json(
        ...
        **kwargs,
        ...
        with Path.open(filepath, mode="r", **kwargs) as infile:
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        Behaves normally except when assigning to an Enum or Parameter. If
        assigning to an Enum an attempt will be made tocast non-enum values to
        the appropriate Enum. If assigning to Parameter, anattempt will be made
        to unpack the values to the appropriate Parameter.
        ...
        if name in self.__dataclass_fields__:
            ...
            if issubclass(value_type, Enum):
                # Syntax for getting by name of enum vs. value of enum is
                # different
                try:
                    value = (
                        value_type[value]
                    ...
                    error_msg = (
                        f"Error setting parameter '{self.__class__.__name__}.{name}'. "
                        "Valid values are: {', '.join(value_type._member_names_)}"
                    )
                    raise KeyError(error_msg) from e
            if is_dataclass(value_type) and not isinstance(value, value_type):
                # NOTE: This does not automatically convert a list of
                #   type list[Parameter]
                ...
                try:
                    ...
                    error_msg = (
                        f"Error setting {self.__class__.__name__}.{name}, initializing "
                        f"in {error_path}: {e.args}"
                    )
                    raise KeyError(error_msg) from e
                value._parent = self
...
class BasePairedParameters(BaseParameters):
    ...
    def validate(self) -> None:
        ...
        if self._match and (self.mvm != self.vmm):
            raise ValueError(
                f"{self.__class__} parameters set to match, but different"
                " values given.",
            )
    ...
    def match(self, value: bool) -> None:
        ...
        appropriately. When syncing, mvm takes precedence over vmm.
        """
        self._match = value
        if self._match:
            self.vmm = self.mvm
...
def _resolve_type(value_type: Any, module_name) -> type:
    """Returns the type of the corresponding type name from the module.

    This allows casting to be done in the BaseParameter object without any extra
    boilerplate on any other subclassed parameter object.

    Because the value to be cast as could be subclassed anywhere else the type
    is not in the namespace on loading. This function uses the module of origin
    to retrieve the constructor for that type.
    ...
    Args:
        value_type (Any): Either a string of the type's name, or the type
            itself.
    ...
    if isinstance(value_type, str):
        # This is a workaround to handle *some* generics until the issues with
        # PEP 563 are resolved in a future version of python
