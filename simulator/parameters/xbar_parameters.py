...
class XbarParameters(BaseParameters):
    ...
    def validate(self) -> None:
        ...
        if not self.array.parasitics.current_from_input and (
            ...
            raise ValueError(
                "If current_from_input=False, must use input bit slicing with "
                "slice_size = 1."
            )
...
class DeviceParameters(BaseParameters):
    ...
    Attributes:
        ...
        Vread (float): Voltage used to convert conductance to current during
            MVM. This is currently used for SONOS only
        ...
        programming_error (WeightErrorParameters): Parameters for device
            programming error
        drift_error (DriftErrorParameters):# Parameters for device conductance
            drift
...
class ArrayParameters(BaseParameters):
    ...
    Attributes:
        Icol_max (float): Maximum current in a column, in units of the maximum
            current that can be drawn by a single device in the array. Any
            column current that exceeds (-Icol_max, +Icol_max) will be clipped
            to these bounds
...
class PairedADCParameters(BasePairedParameters):
    ...
    # TODO: ADC type changer for 3.x, will be replaced in 4.0
    def _change_adc_type(self):  # noqa: C901
        """Swaps ADC parameters when the ADC model changes."""
        if (
...
class ADCParameters(BaseParameters):
    ...
    # TODO: Quick little hack for swapping param objects, just till 4.x changes
    def __setattr__(self, name: str, value: Any) -> None:
        """Swaps ADC paramters on ADC model change."""
    ...
    # TODO: Temporary for 3.x
    @classmethod
    def get_adc_type(cls, model: str):
        """Swaps parameter types based on the selected ADC type."""
        if model == "RampADC":
...
class RampADCParameters(ADCParameters):
    ...
    Attributes:
        ...
        sigma_capacitor (float): Standard deviation of the random variability in
            the minimum-sized capacitor in the CDAC, normalized by the minimum
            capacitance value.
        sigma_comparator (float): Standard deviation of the random variability
            ...
            comparison. There is a comparator associated with every array column
            (MVM) and/or row (VMM). The offset is normalized by the reference
            voltage used for the ramp.
        symmetric_cdac (bool): Whether to use the symmetric CDAC design that
            treats the ADC levels as two's complement signed integers. If False,
            uses an alternative CDAC design that treats the ADC levels as
            unsigned integers.
    """
...
class SarADCParameters(ADCParameters):
    ...
    Attributes:
        gain_db (float): Open-loop gain in decibels of the operational amplifier
            at the output of the capacitive DAC (CDAC)
        sigma_capacitor (float): Standard deviation of the random variability in
            the minimum-sized capacitor in the CDAC, normalized by the minimum
            capacitance value.
        sigma_comparator (float): Standard deviation of the random variability
            in the input offset voltage of the comparator. The comparator
            compares the analog ADC input to the analog CDAC output during each
            SAR cycle. There is a comparator for every group of ADC inputs.
        split_cdac (bool): Whether to use the split capacitor CDAC design to
            reduce the average size of the capacitor in the CDAC.
        group_size (int): Number of ADC inputs that share a SAR unit. Inputs
            within and group use the same CDAC and comparator. This corresponds
            to the number of grouped columns (MVM) or grouped rows (VMM) of the
            array.
    """
...
class PipelineADCParameters(ADCParameters):
    ...
    Attributes:
        gain_db (float): Open-loop gain in decibels of the operational amplifier
            used as the residue amplifier in a 1.5-bit stage of the pipeline
            ADC.
        sigma_C1 (float): Standard deviation of the random variability in
            capacitor C1 used to amplify the voltage by 2X in the 1.5-bit
            switched-capacitor stage. The amplification factor is (1 + C1/C2),
            where C1 = C2 in the ideal case.
        sigma_C2 (float): Standard deviation of the random variability in
            capacitor C2 in the 1.5-bit ADC stage, normalized by the nominal
            value of C2.
        sigma_Cpar (float): Standard deviation of the random variability in the
            parasitic capacitance at the negative input of the operational
            amplifier in the 1.5-bit stage, normalized by the nominal value of
            C1.
        sigma_comparator (float): Standard deviation of the random variability
            in the input offset voltage of the comparators used in the 1.5-bit
            stages.
        group_size (int): Number of ADC inputs that share single pipeline ADC
            and its random capacitor mismatches and comparator offsets.
            This corresponds to the number of grouped columns (MVM) or grouped
            rows (VMM) of the array.
    """
...
class DACParameters(BaseParameters):
    """Parameters for the digital-to-analog converter (DAC).

    DACs are the generic term for the circuit used to quantize the input signals
    that are passed to the array.

    Attributes:
        model (DACModel): name of the model used to specify quantization
            behavior. This must match the name of a child class of IDAC, other
            than "DAC"
        bits (int): bit resolution of the digital input
        input_bitslicing (bool): whether to bit slice the digital inputs to the
            MVM/VMM and accumulate the results from the different input bit
            slices using shift-and-add operations.
        sign_bit (bool): whether the digital input is encoded using
            sign-magnitude representation with a range that is symmetric around
            zero
        slice_size (int): Default slice size for input bit slicing. Can be
            overridden from within the individual cores
    ...
    @property
    def sign_bit(self):
        """Whether the DAC supports signed values."""
        return self.min < 0
...
class WeightErrorParameters(BaseParameters):
    ...
    Attributes:
        ...
        model (WeightErrorModel): Weight error model to use. This must match
            the name of a child class of IDevice, other than "Device",
            "EmptyDevice", and "GenericDevice"
        magnitude (float): Standard deviation of the random conductance error
            that is applied either as programming error or read noise when using
            one of the generic device models. This is normalized either to
            either the maximum device conductance (NormalIndependentDevice) or
            the target device conductance (NormalProportionalDevice)
    """
...
class ParasiticParameters(BaseParameters):
    ...
    Attributes:
        enable (bool): Whether to enable parasitic resistance model. For
            bit sliced,this indicates whether parasitics is enabled for ANY of
            the slices
        Rp_row (float): Parasitic resistance of the row metallic
            interconnects (ohms)
        Rp_col (float): Parasitic resistince of the column metallic
            interconnects (ohms)
        ...
        current_from_input (bool): If True, current is sourced from the same
            line that carries the input signal. If False, current is sourced
            from a separate line and the input signal is used to connect the
            cell to this separate line.
        selected_rows (str): Values: "top" or "bottom". Only used if
            current_from_input = False. If MVM/VMM # inputs < rows_max, this
            sets which rows are selected: "top" means selected rows are far from
            the ADC, "bottom" means they are close to the ADC. The optimal
            choice depends on pos/neg interleaving and whether Rp_col or Rp_row
            is larger.
    """
    ...
    def Rp_row_norm(self) -> float:
        """Row parasitic resistance normalized by the min device resistance."""
        params = self.root
        return self.Rp_row / params.xbar.device.Rmin

    @property
    def Rp_col_norm(self) -> float:
        """Col parasitic resistance normalized by the min device resistance."""
        params = self.root
        return self.Rp_col / params.xbar.device.Rmin

    @property
    def Rp_row_terminal_norm(self) -> float:
        """Row series resistance normalized by the min device resistance."""
        params = self.root
        return self.Rp_row_terminal / params.xbar.device.Rmin

    @property
    def Rp_col_terminal_norm(self) -> float:
        """Col series resistance normalized by the min device resistance."""
        params = self.root
