...
from simulator.parameters.device.types import RRAMMiloParameters

xp = ComputeBackend()


class RRAMMilo(BaseDevice):
    ...
    def __init__(
        ...
        model_params: RRAMMiloParameters,
    ):
    ...
    def programming_error(self, input_):
        ...
        # values
        # Fit coefficients: linear fit to the data in
        # Fig. 4 (IGVVA-100 algorithm)
    ...
    def drift_error(self, input_, time):
        # No drift model
        raise ValueError(
            "RRAM Milo is not a valid drift model. "
            "It can only be used to simulate programming error.",
        )
