...
##### Lumped read noise
Read noise can greatly increase the simulation time, since the random noise must be resampled for every device on every MVM. To speed up these simulations, an approximate model called lumped read noise can be used, by setting ``params.xbar.device.read_noise.lumped = True``. Instead of sampling the noise on every device, the statistics of the accumulated read noise on the dot product current sum will be calculated (which varies across MVMs), then only the noise on these current sums will be sampled. The current sum noise is sampled from a normal distribution, following the central limit theorem, then added to the current sums.

The lumped read noise model can account for the state dependence of the read noise. It can give results that are close to the standard noise model when the MVMs are linear. This means that the approximation becomes worse when strong nonlinear I-V characteristics and/or parasitic resistance effects are included in the simulation. Meanwhile, the standard read noise model is fully compatible with these nonlinear effects.

To ensure that a custom device model is compatible with lumped read noise, a new function called ``read_noise_variance`` must be defined, which returns the variance of the read noise for every device in an array. For devices that have normally distributed read noise for any given state, much of this function may already be implemented within the ``read_noise`` function. See ``SONOS`` for an example.
