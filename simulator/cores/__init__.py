...
# Algorithm -> Wrapper Core (OffsetCore, BalancedCore, or BitslicedCore) ->
# Clipper Core (NumericCore)
...
#   - Scaled from outer [algorithm's] dynamic range (e.g.: -10,10) to wrapper's
#   dynamic range (e.g.: -0.5,0.5)
# 2 Sent to OffsetCore/BalancedCore/BitslicedCore/...
#   - Converted to internal representation as needed to be sent to the inner
#     core(s) (e.g.: positive values to positive core; negative values to
#     negative core)
#     (Note that conversion to the inner dynamic range (to 0,1) is a side-effect
#     of this process)
...
#   - Output is clipped and quantized for VMM and MVM, clipping/quantization is
#     done in offset/balanced core so the multiple outputs can be combined
#     before clipping/quantization
# 6 Returned to OffsetCore/BalancedCore/BitslicedCore
#   - Output from core(s) is combined/post-processed (e.g.: subtraction for
#     OffsetCore)
