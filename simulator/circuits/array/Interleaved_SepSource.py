...
class InterleavedSeparateSourceArray(IArray):
    """This class models an array where:
    1) The current through the memory cells is sourced from a separate
        interconnect than the input signal, and this interconnect is parallel to
        the output summation column. The input signal is used to
        connect/disconnect the summation column to the current source column.
    2) Memory cells for positive and negative weights in a differential pair are
        interleaved, connecting to the same summation column. On the other side,
        the positive cell is connected to a positive voltage and the negative
        cell is connected to a negative voltage w.r.t the summation column.
        This allows current to cancel locally along the summation column.
    ...
    def __init__(
        ...
        """Initialized an interleaved array with seperate sources."""
    ...
    def solve_mvm(  # noqa:C901
        self,
        vector: npt.NDArray,
        matrix_pos: npt.NDArray,
        matrix_neg: npt.NDArray,
        row_in: bool = True,
    ) -> npt.NDArray:
        """Calculates the MVM result including parasitic resistance.

        matrix_pos: normalized conductance matrix for positive conductances
            in diff. pair
        matrix_neg: normalized conductance matrix for negative conductances
            in diff. pair.
        """
        ...
        # If input is zero, device is gated off and has effectively zero
        # conductance
        ...
        Ires_pos = self._cell_currents(matrix_pos, dV0_pos)
        Ires_neg = self._cell_currents(matrix_neg, -dV0_pos)
        ...
        if self.Rp_in_mat is None or self.Rp_out_mat is None:
            #   Rp_out is the parasitic resistance of the interconnect that
            #      carries the output partial sums
            #   Rp_in is the parasitic resistance of the interconnect on other
            #      side, supplying or sinking current
            if row_in:
            ...
            self.Rp_out_mat = Rp_out * xp.ones((
                matrix_pos.shape[0],
                matrix_pos.shape[1],
            ))
        ...
        while Verr > self.Verr_th and Niters < self.Niters_max:
            ...
            if len(Isum_col.shape) == 4:
                ...
                Vpar_pos = Vdrops_col + xp.cumsum(
                    self.Rp_in_mat[:, :, None, None] * Isum_pos, 1
                )
                Vpar_neg = Vdrops_col + xp.cumsum(
                    self.Rp_in_mat[:, :, None, None] * Isum_neg, 1
                )
            else:
            ...
            Ires_pos = self._cell_currents(matrix_pos, dV_pos)
            Ires_neg = self._cell_currents(matrix_neg, dV_neg)
            Ires = Ires_pos + Ires_neg
            Niters += 1
