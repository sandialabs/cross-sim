...
class InterleavedInputSourceArray(IArray):
    ...
    def __init__(
        ...
        """Initialized an interleaved array with a single source."""
    ...
    def solve_mvm(  # noqa:C901
        ...
        while Verr > self.Verr_th and Niters < self.Niters_max:
            # Calculate parasitic voltage drops (reuse variable names to reduce
            # memory usage)
            ...
            if len(Ires.shape) == 4 and isinstance(self.Rp_out_mat, xp.ndarray):
                Vdrops_col = xp.cumsum(
                    (self.Rp_out_mat[:, :, None, None] * Ires)[:, ::-1], 1
                )[:, ::-1]
            else:
            ...
            if len(Ires_pos.shape) == 4 and isinstance(self.Rp_in_mat, xp.ndarray):
                Vpar_pos = Vdrops_col + xp.cumsum(
                    self.Rp_in_mat[:, :, None, None] * Ires_pos, 0
                )
                Vpar_neg = Vdrops_col + xp.cumsum(
                    self.Rp_in_mat[:, :, None, None] * Ires_neg, 0
                )
            else:
